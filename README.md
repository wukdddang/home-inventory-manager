...
- **프론트엔드**: 주요 화면 구현 완료 (대시보드, 구매, 재고 이력, 설정, 알림). 백엔드 API 연동 완료 (101개+ 엔드포인트). `/mock` 경로에서 localStorage 기반 데모 모드도 제공
- **백엔드**: 엔티티 21개, 컨트롤러 17개, 비즈니스 서비스 17개 모듈 구성 및 서비스 로직 구현 완료. JWT 인증, 역할 기반 가드, PostgreSQL 연동
- **E2E 테스트**: 12개 유스케이스 작성 완료 (인증, 비밀번호, 거점, 카탈로그, 재고, 구매, 소비/폐기, 장보기, 초대, 알림, 거점 삭제, 토큰 갱신). 실제 백엔드 + DB 환경에서 검증
