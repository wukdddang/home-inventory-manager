...
| **조회** | 저장된 구조를 API로 불러와, 그 위에 배치한 물품을 시각적으로 확인 |
| **연동** | 방(또는 슬롯) 클릭 시 해당 StorageLocation의 물품 목록/정보 표시 |
...
### 3.3 확정 스코프: ERD 스타일 + 이름 변경 + 방 클릭 시 물품 정보
...
- **UX**: **방 클릭** → 그 방(StorageLocation)에 연결된 **물품 목록/정보**를 패널/모달로 표시.
...
- **현재 스코프(이름 변경 + 방 클릭 → 물품 정보)**  
  → **SVG + React**로 구현 권장.  
  - `rooms: [{ id, name, x, y, width, height }]` 상태로 각 방을 `<rect>` + 라벨 렌더.  
  - **클릭** 시 선택 상태 저장 → 우측/하단 패널에 해당 방(StorageLocation)의 **물품 목록** 표시.  
...
5. **(선택) 3D 뷰**  
   - Three.js(R3F)로 동일 `structurePayload` 렌더, 방/슬롯 클릭 시 물품 정보 표시.
