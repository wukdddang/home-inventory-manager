...
| 트리 | URL 역할 |
|------|-----------|
| `app/(current)/…` | **실제** 라우트 (`/login`, `/signup`, `/dashboard`, `/settings` …). `page.tsx`는 `(mock)/mock`의 동일 경로 `page`를 **re-export**만 함. |
| `app/(mock)/mock/…` | **구현본** + `/mock/…` 라우트 (로그인·회원가입·홈·대시보드·설정 전부). |

공유 UI: `app/(current)/_ui/` (`AppShell`, `AuthGuard`). **`(current)/layout`에는 가드 없음** — `dashboard`·`settings` 세그먼트 `layout.tsx`에서만 `AuthGuard` + `AppShell`. **`/mock/*` 전체에는 AuthGuard를 쓰지 않음**(비로그인 허용, 필요 시 `AppShell`만).
