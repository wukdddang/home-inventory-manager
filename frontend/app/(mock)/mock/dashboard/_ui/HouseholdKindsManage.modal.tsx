...
function HouseholdKindsManageModalBody({
  ...
  const pendingDelete = pendingDeleteId
    ? draft.find((d) => d.id === pendingDeleteId)
    : null;
  ...
  return (
    <>
      <div className="flex max-h-[min(90vh,36rem)] flex-col rounded-2xl border border-zinc-700 bg-zinc-900 shadow-xl">
          ...
          <div className="flex shrink-0 justify-end gap-2 border-t border-zinc-800 p-6 pt-4">
            ...
            <button
              ...
              onClick={handleSave}
              disabled={
                sortedDraft.every((d) => !d.label.trim()) ||
                sortedDraft.some((d) => !d.id.trim())
              }
      ...
      <AlertModal
        ...
        description={
          pendingDelete
            ? `「${pendingDelete.label}」유형을 목록에서 제거할까요? 이 유형을 쓰는 거점은 다른 유형으로 바뀝니다.`
            : undefined
        }
        confirmLabel="삭제"
        cancelLabel="취소"
        ...
        onConfirm={() => {
          if (pendingDeleteId) {
            setDraft((rows) => rows.filter((r) => r.id !== pendingDeleteId));
          }
