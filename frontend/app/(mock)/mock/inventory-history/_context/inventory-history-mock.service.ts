...
/* ═══════════════════════════════════════════════════════════
   거점 스냅샷 (이력 행의 위치 라벨 해석용)
   ═══════════════════════════════════════════════════════════ */

export const MOCK_HISTORY_HOUSEHOLDS: Household[] = [
  {
...
function buildBaseLedgerRows(): InventoryLedgerRow[] {
  const iso = 로컬날짜시각을_만든다;
  return [
    {
      id: "mock-ledger-001",
...
function buildExtraLedgerRows(): InventoryLedgerRow[] {
  const items = [
    {
      inventoryItemId: "mock-item-ramen",
      itemLabel: "식료품 › 라면 › 1봉",
  ...
  for (let i = 0; i < 42; i++) {
    ...
    rows.push({
      ...
      reason: type === "waste" ? wasteReasons[i % 3]! : undefined,
      createdAt: 로컬날짜시각을_만든다(-(1 + (i % 28)), hour, minute),
    });
  }
  return rows;
}

export const MOCK_HISTORY_LEDGER: InventoryLedgerRow[] = [
  ...buildBaseLedgerRows(),
  ...buildExtraLedgerRows(),
];
