...
| 상태 | 항목 수 | 내용 |
|------|---------|------|
| ✅ 완료 | 인프라 6, 사용자 6+1(비밀번호변경), 거점 7, 거점초대 4+2(수락·토큰조회), 거점유형 2, 카테고리 4, 단위 4, 상품 4, 변형 4, 가구 CUD+조회 4, 보관장소 생성+조회+삭제+수정 4, 방/집구조 4, 구매 2+1(재고연결), **재고품목 목록+등록+수량직접수정 3**, 소비/폐기이력 2, **이력조회+수동조정 2**, **로트 목록+임박+만료 3**, 장보기 5, 알림 2, 알림설정 3, 만료규칙 4, 장보기자동제안 2, 스케줄러 2 | **총 ~100항목** |
| 🚧 미연결 | — | **0항목** |
