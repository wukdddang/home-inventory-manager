...
| 재고 수량 수정 (직접 set) | `PATCH /api/households/[id]/inventory-items/[iid]/quantity` | ✅ | `DashboardContext` `재고_수량을_직접_설정_한다` → `port.updateInventoryItemQuantity` — `ItemDetailDrawer` 수량 카드 ✏️ 버튼으로 절댓값 입력, 메모 선택 입력, 성공 시 ledger `adjust` 기록 |
...
| 유통기한 임박 목록 | `GET /api/households/[id]/batches/expiring` | ✅ | `DashboardContext` `거점_유통기한_임박_만료_를_로드_한다` — 거점 로드 후 API 호출, purchases와 조인하여 `ExpiryAlertItem[]` 생성, `거점_유통기한_임박_목록을_가져온다`로 노출. `ExpiryAlerts.section` (모바일 패널)에서 context 구독 |
| 만료된 목록 | `GET /api/households/[id]/batches/expired` | ✅ | `DashboardContext` `거점_유통기한_임박_만료_를_로드_한다` — 동일 로직으로 `거점_만료_목록을_가져온다`로 노출. mock 모드는 `getMockPurchasesSession()` 기반 필터링 |
...
| 알림 설정 삭제 | `DELETE /api/notification-preferences/[id]` | ✅ | `알림_설정을_초기화_한다` → `port.deleteNotificationPreference` — `NotificationSettings.section` 하단 "초기화" 버튼으로 2단계 확인 후 호출, 로컬 상태 기본값 복원 |
...
| 완료일 | 원인 / 항목 |
|--------|-------------|
| 2026-03-31 | ~~방/공간 편집기와 백엔드 모델 불일치~~ — 방 목록 조회·동기화, 집 구조 조회·저장 (4항목) |
| 2026-03-31 | ~~가구·보관장소 write-through 미구현~~ — 가구 CUD, 보관장소 생성·삭제·수정 (6항목) |
| 2026-03-31 | ~~재고 품목 API 연결 미완료~~ — 재고 품목 목록·등록 (2항목) |
| 2026-03-31 | ~~로트 별도 조회 미연결~~ — 로트 목록 (1항목) |
| 2026-03-31 | ~~이력 조회 API 미연결~~ — 재고 이력 조회·수동조정 (2항목) |
| 2026-03-31 | ~~인증 UI 미구현~~ — 비밀번호 변경, 초대 링크 수락·토큰 조회 (3항목) |
| 2026-03-31 | ~~보관장소 이름 수정 UI 없음~~ — 보관장소 수정 (1항목) |
| 2026-03-31 | ~~직접 수량 set UI 없음~~ — `ItemDetailDrawer` ✏️ 버튼으로 절댓값 입력 UI 추가, `재고_수량을_직접_설정_한다` → `PATCH /quantity` (1항목) |
| 2026-03-31 | ~~유통기한 임박/만료 전용 API 미연결~~ — `DashboardContext` `거점_유통기한_임박_만료_를_로드_한다`에서 `batches/expiring`, `batches/expired` 호출, `ExpiryAlerts.section` context 구독으로 교체 (2항목) |
| 2026-03-31 | ~~알림 설정 삭제 UI 없음~~ — `NotificationSettings.section` 하단 '초기화' 버튼, `알림_설정을_초기화_한다` → `DELETE /api/notification-preferences/[id]` 2단계 확인 UI 추가 (1항목) |
...
4. ~~**유통기한 임박/만료 전용 API** — 완료 (2026-03-31)~~
5. ~~**재고 수량 직접 수정** — 완료 (2026-03-31)~~
6. ~~**가구·보관장소 CUD** — 완료 (2026-03-31)~~
7. ~~**방/구조** — 완료 (2026-03-31)~~
8. ~~**알림 설정 삭제 UI** — 완료 (2026-03-31)~~
