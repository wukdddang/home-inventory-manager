...
|---|------------|---------------|------|------|--------|------|
| 2-1 | `Household` | **`kind`** | Household에 `kind varchar nullable` 유지 + **HouseholdKindDefinition 테이블 추가** | 프론트가 거점 유형(home/office/vehicle/other)으로 적극 사용 중. 라벨·순서 관리를 위해 **사용자별 HouseholdKindDefinition 테이블 추가 결정** (v2.3). Household.kind는 kindId 값을 저장, FK 제약 없이 느슨한 참조 | **P1** | **확정 (v1.6 갱신)** |
